 ...
 */
tir::Stmt MakeDeviceHookCall(const std::string& device_name, const std::string& hook, PrimExpr context) {
...
class AOTCallGenerator {
 public:
  AOTCallGenerator(std::string func_name) :
  ...
  void PushStackDLTensor(const TensorType& ttype, PrimExpr data) {
    ...
    prep_stmts_.push_back(
      tir::Evaluate(
        tvm::tir::Call(DataType::Handle(), tvm::tir::builtin::tvm_struct_set(),
                       {dltensor_var, 0, tir::builtin::kArrTypeCode, IntImm(DataType(kDLUInt, 8, 1), ttype->dtype.code())})));
    prep_stmts_.push_back(
      tir::Evaluate(
        tvm::tir::Call(DataType::Handle(), tvm::tir::builtin::tvm_struct_set(),
                       {dltensor_var, 0, tir::builtin::kArrTypeBits, IntImm(DataType(kDLUInt, 8, 1), ttype->dtype.bits())})));
    prep_stmts_.push_back(
      tir::Evaluate(
        tvm::tir::Call(DataType::Handle(), tvm::tir::builtin::tvm_struct_set(),
                       {dltensor_var, 0, tir::builtin::kArrTypeLanes, IntImm(DataType(kDLUInt, 16, 1), ttype->dtype.lanes())})));

    // Populate DLTensor.shape
    for (size_t i = 0; i < ttype->shape.size(); ++i) {
      prep_stmts_.push_back(
        tvm::tir::Store(shape_var, IntImm(DataType(kDLInt, 64, 1), Downcast<IntImm>(ttype->shape[i])->value), IntImm(DataType(kDLUInt, 64, 1), i), tir::const_true()));
    }
  ...
  tir::Stmt GenerateUnpacked(std::string device_name, PrimExpr device_context) {
    auto make_call = [this] {
      return tir::Evaluate(tvm::tir::Call(DataType::Int(32), tvm::tir::builtin::call_extern(), args_));
    };
...
class AOTExecutorCodegen : public MixedModeVisitor {
 protected:
  ...
  void CreateFuncCall(CallLoweredProps call_lowered_props, const Expr& result_expr) {
    AOTCallGenerator call_gen{call_lowered_props.lowered_func->name_hint};
    ...
    for (const Expr& arg : call_lowered_props.arguments) {
      if (params_by_expr_.find(arg) != params_by_expr_.end()) {
        ...
        // needs to be removed.
        call_gen.PushStackDLTensor(Downcast<TensorType>(arg->checked_type()),
                                   tvm::tir::Cast(DataType::Handle(32, 1), param_handle));
      } else {
        auto sids = FindExpr(arg);
        call_gen.PushStackDLTensors(arg, sids);
      }
    }

    // Pack the return(s) value. A call node can produce multiple outputs
    auto result_expr_sid = PackSid(result_expr);
    call_gen.PushStackDLTensors(result_expr, result_expr_sid);
    ...
    case CallType::kUnpacked: {
      ICHECK(has_c_device_api_context) << "Only support unpacked calls with device context";
      // call_extern calling convention with optional context
      auto device_context = device_contexts_.Get(global_var).value();
      std::string device_name = FindDeviceName(device_context);
      body = call_gen.GenerateUnpacked(device_name, device_context);
      break;
    }
    case CallType::kCPacked:
      ICHECK(!has_c_device_api_context) << "Only support CPacked calls with no device context";
      body = call_gen.GenerateCPacked();
      break;
 ...
 public:
  ...
  LoweredOutput Codegen(IRModule mod, relay::Function func, String mod_name) {
    ...
    Executor executor_config = mod->GetAttr<Executor>(tvm::attr::kExecutor).value();
    std::string interface_api = executor_config->GetAttr<String>("interface-api").value_or("packed");
    Integer workspace_byte_alignment =
        executor_config->GetAttr<Integer>("workspace-byte-alignment").value_or(16);
    ...
    if (runtime_config->name == kTvmRuntimeCrt) {
      if (unpacked_api == true && interface_api == "c") {
        call_type_ = CallType::kUnpacked;
      } else if (unpacked_api == false && interface_api == "c") {
        ...
        CHECK(interface_api == "packed" || unpacked_api == true)
          << "Either need interface-api == \"packed\" (got: " << interface_api
          << ") or unpacked-api == true (got: " << unpacked_api
