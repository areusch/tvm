...
class ConvertRelaxMainToDPS : public ExprMutator {
 public:
  ...
  IRModule operator()() {
    ...
    Function new_func = Function(new_params, new_body, TensorStructInfo(DataType::Int(32), 0), main_func->attrs);
 ...
 private:
  ...
  BindingBlock VisitBindingBlock(const BindingBlock& block) override {
    ...
    for (auto iter = block->bindings.rbegin(); iter != block->bindings.rend(); iter++) {
      ...
      } else if (binding.as<MatchCastNode>()) {
        LOG(FATAL) << "MatchCast bindings are not supporter for now.";
      } else {
...
tvm::transform::Pass ConvertRelaxMainToDPS(Bool attach_io_to_attrs) {
  ...
  return tvm::transform::CreateModulePass(pass_func, 0, "relax.ConvertRelaxMainToDPS", {});
}
