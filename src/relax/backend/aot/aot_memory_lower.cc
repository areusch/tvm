...
class AOTMemLowerMutator : public ExprMutator {
  ...
  Expr VisitExpr_(const CallNode* call) override {
    ...
    if (call->op == alloc_tensor_op) {
      ...
      DataType dtype = call->args[1].as<DataTypeImmNode>()->value;
      Expr storage_size = ComputeStorageSize(output_shape, dtype);

      Var storage =
          builder_->Emit(Call(memory_alloc_storage_op, Array<Expr>({storage_size, PrimValue(-1), StringImm("global"), DataTypeImm(dtype)}), {}),
                         "storage");
      Expr shape = call->args[0];
      return Call(memory_alloc_tensor_op, Array<Expr>({storage, PrimValue(IntImm(0)), shape, DataTypeImm(dtype)}));
    }
