...
# The Python constraint required by TVM. May be a semver, in which case it will be
# translated to a requirements.txt specifier for placement in setuptools' python_requires.
PYTHON_CONSTRAINT = ">=3.6.1,<4.0"
...
REQUIREMENTS_BY_PIECE: RequirementsByPieceType = [
    ...
    (
        "importer-keras",
        ("Requirements for the Keras importer", ["h5py", "keras", "tensorflow", "tensorflow-estimator"]),
    ),
    ("importer-mxnet", ("Requirements for the MXNet importer", ["mxnet"])),
    (
        "importer-onnx",
        (
    ...
    ("importer-paddlepaddle", ("Requirements for the PaddlePaddle importer", ["paddlepaddle"])),
    (
        "importer-pytorch",
        (
    ...
    (
        "importer-tflite",
        ("Requirements for the TFLite importer", ["keras", "tensorflow", "tensorflow-estimator", "tflite"]),
    ),
    ("tutorial-dgl", ("Requirements for the dgl tutorial", ["dgl"])),
    ("tutorial-gluoncv", ("Requirements for the gluoncv tutorial", ["gluoncv"])),
    (
        "tvmc",
        (
...
JoinedRequirements = typing.Dict[str, typing.Tuple[str, typing.List[str]]]


def join_requirements() -> JoinedRequirements:
...
AUTOGENERATED_LINE = "AUTOGENERATED SECTION BY python/gen_requirements.py. Do not edit."


GENERATED_SECTION_NAMES = ("tool.poetry.dependencies",
                           "tool.poetry.dev-dependencies",
                           "tool.poetry.extras")


def _generate_tool_poetry_dependencies(joined_deps, pyproject_lines):
    requirements_by_piece_dict = dict(REQUIREMENTS_BY_PIECE)
    pyproject_lines.append("[tool.poetry.dependencies]\n")
    pyproject_lines.append(f"python = \"{PYTHON_CONSTRAINT}\"\n")
    for package, constraint in CONSTRAINTS:
        if constraint is None:
            constraint = "*"
        if package not in requirements_by_piece_dict["dev"][1]:
            if package in requirements_by_piece_dict["core"][1]:
                pyproject_lines.append(f"{package} = \"{constraint}\"\n")
            else:
                version_str = f"version = \"{constraint}\", optional = true"
                pyproject_lines.append(f"{package} = {{ {version_str} }}\n")


def _generate_tool_poetry_dev_dependencies(joined_deps, pyproject_lines):
    requirements_by_piece_dict = dict(REQUIREMENTS_BY_PIECE)
    pyproject_lines.append("[tool.poetry.dev-dependencies]\n")
    for package, constraint in CONSTRAINTS:
        if constraint is None:
            constraint = "*"

        if package in requirements_by_piece_dict["dev"][1]:
            pyproject_lines.append(f"{package} = \"{constraint}\"\n")


def _generate_tool_poetry_extras(joined_deps, pyproject_lines):
    pyproject_lines.append("[tool.poetry.extras]\n")
    requirements_by_piece_dict = dict(REQUIREMENTS_BY_PIECE)
    for piece, (_, dependencies) in requirements_by_piece_dict.items():
        if piece in ("core", "dev"):
            continue

        pyproject_lines.append(f"{piece} = {dependencies!r}\n")


SECTION_RE = re.compile(r"^\[([^\]]+)\].*$")


def _modify_pyproject_toml(joined_deps : JoinedRequirements, pyproject_toml_path : str):
    """Rewrite sections of pyproject.toml according to --modify-pyproject-toml."""
    pyproject_lines = []
    with open(pyproject_toml_path) as pyproject_toml_f:
        discard_section = False
        for line in pyproject_toml_f:
            stripped_line = line.rstrip("\r\n")
            m = SECTION_RE.match(line)
            if m:
                if m.group(1) in GENERATED_SECTION_NAMES:
                    python_name = m.group(1).replace('.', '_').replace('-', '_')
                    globals()[f"_generate_{python_name}"](joined_deps, pyproject_lines)
                    discard_section = True
                else:
                    discard_section = False

            if not discard_section:
                pyproject_lines.append(line)

    with open(pyproject_toml_path, "w") as pyproject_toml_f:
        for line in pyproject_lines:
            pyproject_toml_f.write(line)
...
def join_and_write_requirements(args: argparse.Namespace):
    ...
    if args.modify_pyproject_toml:
        _modify_pyproject_toml(joined_deps, args.modify_pyproject_toml)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=("If given, replace [poetry.tool.dependencies], [poetry.tool.dev-dependencies], "
              "and [poetry.tool.extras] sections of the pyproject.toml file"))
