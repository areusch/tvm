...
# The tutorial pages in the documentation are generated by Python scripts using Sphinx-Gallery.
# These Python scripts are compiled and run against the TVM build.

from sphinx_gallery.sorting import ExplicitOrder
import glob

# The sub-directories of this `docs` directory which may contain tutorials. To include a tutorial,
# place it at a path like `docs/<subsection>/.../tutorials-src/import_a_model.py`, and ensure that
# `<subsection>` is in this list.
section_dirs = [
    "deploy",
    "describe_models",
    "import_models",
...
examples_dirs = [g for s in section_dirs for g in glob.glob(f'{s}/**/tutorials-src') + glob.glob(f'{s}/tutorials-src')]
print('examples dirs', examples_dirs)
gallery_dirs = [e[:-len("-src")] for e in examples_dirs]
