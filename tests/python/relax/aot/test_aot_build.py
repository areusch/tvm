...
from tvm.relax import build as vm_build
...
def _export_mod(mod):
    ...
    wrkspc = "./aot_workspace/"
    test_so_path = temp_dir / "test.so"
    mod.export_library(test_so_path, cc="gcc", options=["-std=c11", "-g3", "-O0"], workspace_dir=wrkspc)
...
def test_concatenate(enable_usmp):
    dtype = "int32"
    target = "llvm"
    inputs = {
        "x": np.array([[-10, 1], [5, 1]], dtype=dtype),
    ...
    def _relay():
        x = relay.var("x", shape=(2, 2), dtype=dtype)
        y = relay.var("y", shape=(2, 2), dtype=dtype)
        out = relay.concatenate(relay.Tuple([x, y]), axis=0)
        return relay.Function(relay.analysis.free_vars(out), out)
    ...
    relax_mod = relay_translator.from_relay(
        relay.transform.InferType()(tvm.IRModule.from_expr(_relay()))["main"],
        target,
    )
